...
try:
 ...
 graphviz = None # Handle cases where graphviz is not installed
...
def build_system_graph(root_dir: Path) -> Tuple[Set[str], Dict[str, List[Dict[str, str]]], Dict[str, str]]:
    """
    Scans a directory to build a complete map of all artifact IDs and their synergy links.
    Returns IDs, the graph structure, and a map of artifact statuses.
    """
    ids = set()
    graph = {}
    statuses = {}
    id_pattern = re.compile(r"^\s*\|\s*\*\*Artifact_ID\*\*\s*\|\s*`?([A-Z]+\.[A-Za-z0-9]+\.[A-Za-z0-9_]+)`?\s*\|", re.MULTILINE)
    status_pattern = re.compile(r"^\s*\|\s*\*\*Status\*\*\s*\|\s*(\[.*?\])\s*\|", re.MULTILINE)
    ...
    for md_file in root_dir.rglob("*.md"):
        try:
            ...
            if id_match:
                ...
                if status_match:
                    statuses[artifact_id] = status_match.group(1)
...
def find_governance_path(
    ...
    # Build a reverse graph for efficient traversal, including relation type
    reverse_graph = {node: [] for node in system_graph}
    ...
    # BFS queue will store paths (lists of nodes)
    queue = [[start_node]]
    visited = {start_node}
    ...
    while queue:
        ...
        if current_node.startswith("GVRN.") or current_node.startswith("CORE."):
            return path # Found a governance path
...
def generate_graph_visualization(
 ...
 return
    ...
    dot = graphviz.Digraph('PhoenixLoom', comment='The Cognitive Loom of the Synarche', node_attr={'fontname': 'Helvetica'}, graph_attr={'concentrate': 'true'})
    ...
    shape_map = {
        'UMB': 'box', 'AOP': 'oval', 'GUCA': 'parallelogram', 'SELT': 'note',
        'CSL': 'septagon', 'GVRN': 'doubleoctagon', 'CORE': 'tripleoctagon', 'DEFAULT': 'box'
    }

    # Define node border styles based on artifact status
    status_style_map = {
        '[DEPRECATED]': 'dashed',
        '[ARCHIVED]': 'dotted',
        '[ACTIVE]': 'solid',
    ...
    # Find and prepare the governance path for highlighting
    ...
    for node_id in system_graph.keys():
        ...
        border_style = status_style_map.get(status, 'solid')

        attrs = {
            'shape': shape,
            'style': f'rounded,filled,{border_style}',
            'fillcolor': color
        }

        if node_id == validated_artifact:
            attrs['style'] = f'bold,rounded,filled,{border_style}'
            attrs['color'] = 'blue'
    ...
    for source_id, links in system_graph.items():
        for link in links:
            ...
            edge_attrs = {'label': relation_type, 'fontcolor': edge_category_colors.get(category, 'black')}

            if (source_id, link['target']) in path_edges or (link['target'], source_id) in path_edges:
                edge_attrs['color'] = 'orange'
                edge_attrs['penwidth'] = '2.0'
            else:
                edge_attrs['color'] = edge_category_colors.get(category, 'black')
            dot.edge(source_id, link['target'], **edge_attrs)
...
def main():
	...
	known_ids, system_graph, statuses = build_system_graph(target_file.parent) # Scan the directory of the target file
