...
from typing import Any, cast, Dict, List, Tuple, Set, Optional
...
HIGHLIGHT_STYLE = 'bold,rounded,filled'
...
def load_artifact_schema(ontology_path: Path) -> Dict[str, Any]:
    ...
    content = ontology_path.read_text(encoding="utf-8")
    match = re.search(
        r"## 28\.0\[MACHINE_READABLE_SCHEMAS\].*?```json\s*(\{[^}]*\})\s*```",
        content,
        re.DOTALL,
    )
    if not match:
...
class ArtifactValidator:
    ...
    def parse_artifact(self, file_path: Path) -> Tuple[Dict[str, Any], str]:
        ...
        try:
            ...
            metadata_str, body = self._extract_frontmatter(content, file_path.name)
            if metadata_str is None:
                return {}, ""

            metadata = self._parse_metadata_block(metadata_str)
            return metadata, body
        except Exception as e:
            self._add_error(f"Failed to parse artifact '{file_path.name}': {e}")
            return {}, ""

    def _extract_frontmatter(self, content: str, file_name: str) -> Tuple[Optional[str], str]:
        match = re.search(r'^---\s*\n(.*?)\n---\s*\n', content, re.DOTALL)
        if not match:
            self._add_error(f"File '{file_name}' is missing YAML frontmatter block.")
            return None, ""
        return match.group(1), content[match.end():]

    def _parse_metadata_block(self, metadata_str: str) -> Dict[str, Any]:
        ...
        for line in metadata_str.strip().split('\n'):
            line = line.strip()
            if self._should_skip_metadata_line(line):
                continue

            block_candidate = self._try_parse_block_header(line, block_key_map)
            if block_candidate:
                current_block = block_candidate
                metadata[current_block] = [] if current_block == 'Synergy_Block_D' else {}
                continue

            if current_block and self._is_metadata_row(line):
                self._process_metadata_row(metadata, current_block, line)

        return metadata

    def _should_skip_metadata_line(self, line: str) -> bool:
        return not line or line.startswith('#')

    def _try_parse_block_header(self, line: str, block_key_map: Dict[str, str]) -> Optional[str]:
        if not line.startswith('### Block'):
            return None
        current_block_name = line.split(':', 1)[0].strip()
        return block_key_map.get(current_block_name)

    def _is_metadata_row(self, line: str) -> bool:
        return '|' in line and '---' not in line and 'Key' not in line

    def _process_metadata_row(self, metadata: Dict[str, Any], current_block: str, line: str) -> None:
        parts = [p.strip() for p in line.split('|') if p.strip()]
        if len(parts) < 2:
            return

        key, value = parts[0], parts[1]
        if current_block == 'Synergy_Block_D' and len(parts) == 3:
    ...
    def validate_structure(self, metadata: Dict[str, Any], file_name: str) -> None:
        """
        ...
        try:
            ...
            self._add_error(
                f"Structural validation failed for '{file_name}': {e.message} in {list(e.path)}"
            )
        except Exception as e:
            self._add_error(
                f"An unexpected error occurred during structural validation of '{file_name}': {e}"
            )

    def validate_governance(self, metadata: Dict[str, Any], body: str, file_name: str) -> None:
        """
        ...
        if artifact_id and not re.match(r'^[A-Z]+\.[A-Za-z0-9]+\.[\w]+$', artifact_id):
    ...
    def validate_links(self, metadata: Dict[str, Any], file_name: str) -> None:
    ...
    def validate_loom_connectivity(self, metadata: Dict[str, Any], file_name: str) -> None:
        ...
        if not metadata.get('Synergy_Block_D'):
            self._add_warning(
                f"Loom validation for '{file_name}': Artifact has no outgoing links, potentially violating LAW_021."
            )
...
def build_system_graph(root_dir: Path) -> Tuple[Set[str], Dict[str, List[Dict[str, str]]]]:
    ...
    id_pattern = re.compile(r"^\s*\|\s*\*\*Artifact_ID\*\*\s*\|\s*`?([A-Z]+\.[A-Za-z0-9]+\.[\w]+)`?\s*\|", re.MULTILINE)
    synergy_pattern = re.compile(r"\|\s*`?([A-Z]+\.[A-Za-z0-9]+\.[\w]+)`?\s*\|\s*`?([A-Z_]+)`?\s*\|", re.MULTILINE)
...
def _build_reverse_graph(system_graph: Dict[str, List[Dict[str, str]]]) -> Dict[str, List[Dict[str, str]]]:
    reverse_graph: Dict[str, List[Dict[str, str]]] = {node: [] for node in system_graph}
    for source, links in system_graph.items():
        for link in links:
            target = link.get("target")
            relation = link.get("relation")
            if isinstance(target, str) and isinstance(relation, str) and target in reverse_graph:
                reverse_graph[target].append({"source": source, "relation": relation})
    return reverse_graph


def _is_governance_node(node_id: str) -> bool:
    return node_id.startswith(("GVRN.", "CORE."))
...
def find_governance_path(
    ...
    """Finds the path from a starting artifact to its governing CORE or GVRN law."""
    if not start_node or start_node not in system_graph:
        return []

    reverse_graph = _build_reverse_graph(system_graph)
    queue = [[start_node]]
    visited = {start_node}
    ...
    while queue:
        ...
        if _is_governance_node(current_node):
            return path

        for edge in reverse_graph.get(current_node, []):
            neighbor = edge["source"]
            if neighbor not in visited and edge.get("relation") == "IS_GOVERNED_BY":
...
def generate_graph_visualization(
    ...
    try:
        ...
        print("❌ Graphviz is not installed. Run `pip install graphviz` to enable visualization.")
        return
    ...
    # Define edge colors based on OSLM Relational Grammar
    edge_category_colors = {
        'STRUCTURAL': 'blue', 'GOVERNANCE': 'darkgreen', 'KINETIC': 'orange', 'SYNERGISTIC': 'purple'
    }
    relation_to_category = {
        'CONTAINS': 'STRUCTURAL', 'IS_A_COMPONENT_OF': 'STRUCTURAL', 'UPGRADES': 'STRUCTURAL', 'EXTENDS': 'STRUCTURAL',
        'GOVERNS': 'GOVERNANCE', 'IS_GOVERNED_BY': 'GOVERNANCE', 'MONITORS': 'GOVERNANCE', 'REMEDIATES': 'GOVERNANCE',
        'TRIGGERS': 'KINETIC', 'INVOKES': 'KINETIC', 'ENABLES': 'KINETIC', 'CONSUMES_DATA_FROM': 'KINETIC', 'FEEDS_DATA_TO': 'KINETIC',
        'SYNERGY': 'SYNERGISTIC', 'ENHANCES': 'SYNERGISTIC', 'RESOLVES_DISSONANCE_OF': 'SYNERGISTIC', 'RESONATES_WITH': 'SYNERGISTIC', 'BIRTHED_BY': 'SYNERGISTIC'
    }

    governance_path = find_governance_path(system_graph, validated_artifact)
    path_edges = set(zip(governance_path, governance_path[1:]))
    error_nodes = _collect_error_nodes(errors)

    _add_graph_nodes(dot, system_graph, validated_artifact, governance_path, error_nodes, shape_map)
    _add_graph_edges(dot, system_graph, path_edges, relation_to_category, edge_category_colors)
...
def _add_graph_nodes(
    dot: Any,
    system_graph: Dict[str, List[Dict[str, str]]],
    validated_artifact: Optional[str],
    ...
    for node_id in system_graph.keys():
        ...
        attrs = {
            ...
            'fillcolor': _determine_node_color(node_type),
        }

        if node_id == validated_artifact:
            attrs['style'] = HIGHLIGHT_STYLE
            attrs['color'] = 'blue'
        elif node_id in governance_path:
            attrs['color'] = 'orange'
            attrs['style'] = HIGHLIGHT_STYLE
...
def _add_graph_edges(
    ...
    for source_id, links in system_graph.items():
        for link in links:
            ...
            category = relation_to_category.get(relation_type, 'STRUCTURAL')
            edge_attrs = {
                'label': relation_type,
                'fontcolor': edge_category_colors.get(category, 'black'),
            }

            if (source_id, target_id) in path_edges or (target_id, source_id) in path_edges:
                edge_attrs['color'] = 'orange'
                edge_attrs['penwidth'] = '2.0'
            else:
                edge_attrs['color'] = edge_category_colors.get(category, 'black')
            dot.edge(source_id, target_id, **edge_attrs)


def _collect_error_nodes(errors: Optional[List[str]]) -> Set[str]:
    nodes: Set[str] = set()
    for error in (errors or []):
        if "' does not correspond" in error:
...
def _determine_node_color(node_type: str) -> str:
    if node_type == 'GVRN':
        return 'lightgreen'
    if node_type == 'CORE':
        return 'gold'
