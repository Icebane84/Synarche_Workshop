...
def generate_graph_visualization(
    ...
    dot = graphviz.Digraph('PhoenixLoom', comment='The Cognitive Loom of the Synarche')
    dot.attr(rankdir='LR', size='12,8', splines='true', overlap='false', nodesep='0.5')

    # Define node styles
    node_attrs = {'shape': 'box', 'style': 'rounded,filled', 'fontname': 'Helvetica'}
    error_nodes = {re.search(r"'([^']*)'", e).group(1) for e in (errors or []) if "' does not correspond" in e}

    for node_id in system_graph.keys():
        color = 'lightblue'
        if 'GVRN' in node_id: color = 'lightgreen'
        if 'CORE' in node_id: color = 'gold'
