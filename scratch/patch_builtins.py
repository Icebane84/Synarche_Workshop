...
for i, line in enumerate(lines):
    ...
    if "# Signature of `dict.fromkeys` should be kept identical to" in line:
        new_lines.append(
            "    # Signature of `dict.fromkeys` refined to support subclass type inference.\n"
        )
        new_lines.append(
            "    # Base dict case is handled via specific overloads to maintain parameter precision.\n"
        )
        new_lines.append(
            "    # Subclass cases use Self to ensure the derived class type is preserved.\n"
        )
        new_lines.append("    @classmethod\n")
        new_lines.append("    @overload\n")
        new_lines.append(
            "    def fromkeys(cls: type[dict[Any, Any]], iterable: Iterable[_T], value: None = None, /) -> dict[_T, Any | None]:\n"
        )
        new_lines.append(
        ...
        new_lines.append("    @overload\n")
        new_lines.append(
            "    def fromkeys(cls: type[dict[Any, Any]], iterable: Iterable[_T], value: _S, /) -> dict[_T, _S]:\n"
        )
        new_lines.append(
            '        """Create a new dictionary with keys from iterable and values set to value."""\n'
        )
