...
for i, line in enumerate(lines):
    ...
    if '# Signature of `dict.fromkeys` refined' in line or (3500 < i < 3700 and 'def fromkeys(cls,' in line):
        ...
        j = i + 1
        while j < len(lines) and (not lines[j].strip().startswith('def ') or 'fromkeys' in lines[j]):
             j += 1
        skip_until = j - 1
        continue
