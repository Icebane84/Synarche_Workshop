...
import {
  emitLanguageChangeEnd,
  emitLanguageChangeStart,
        ...
        get(target, prop) {
          ...
          if (typeof prop === "string") {
            ...
            if (accessCounts.current[key] > 1000) {
              console.error(
                `[useTranslation] INFINITE LOOP DETECTED on key: "${key}". Breaking recursion.`,
              );
          ...
          const result = i18nTranslateCopy(currentPath, {
            returnObjects: true,
          });
