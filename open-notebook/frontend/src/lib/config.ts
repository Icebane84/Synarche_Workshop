...
async function fetchConfig(): Promise<AppConfig> {
  ...
  try {
    if (isDev)
      console.log(
        "🔧 [Config] Attempting to fetch runtime config from /config endpoint...",
      );
    ...
    if (runtimeResponse.ok) {
      ...
      if (isDev)
        console.log(
          "⚠️ [Config] Runtime config endpoint returned status:",
  ...
  if (isDev)
    console.log(
      "🔧 [Config] NEXT_PUBLIC_API_URL from build:",
      envApiUrl || "(not set)",
    );
  ...
  if (isDev) {
    ...
    console.log(
      "🔧 [Config] Selection priority: runtime=" +
        (runtimeApiUrl ? "✅" : "❌") +
        ", build-time=" +
        (envApiUrl ? "✅" : "❌") +
        ", smart-default=" +
  ...
  try {
    if (isDev)
      console.log(
        "🔧 [Config] Fetching backend config from:",
        `${baseUrl}/api/config`,
      );
