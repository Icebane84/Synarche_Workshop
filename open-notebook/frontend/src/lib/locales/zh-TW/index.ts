export const zhTW = {
  ...
  sources: {
    ...
    deleteSuccess:
      "來源刪除成功。注意：要從儲存中刪除檔案，必須在設定頁面中啟用「刪除檔案」選項。",
    ...
    notEmbeddedDesc:
      "此內容尚未為了向量搜尋進行嵌入。嵌入可以啟用進階搜尋功能並更好地發現內容。",
  ...
  searchPage: {
    ...
    noEmbeddingModel:
      "您無法使用此功能，因為尚未選擇嵌入模型集。請在模型頁面設定一個。",
  ...
  podcasts: {
    ...
    workflowHint:
      "單集簡介按名稱引用發言人簡介，因此從發言人開始可以避免以後缺少聲音指派。",
    failedToLoadTemplates: "載入模板資料失敗",
    failedToLoadTemplatesDesc:
      "請確保 API 正在運行並重試。某些部分可能不完整。",
    ...
    deleteProfileDesc:
      "這將移除 “{name}”。現有單集將保留其資料，但新單集將不再使用此設定。",
    outlineModel: "大綱模型",
  ...
  advanced: {
    ...
    rebuildWarning:
      "此操作可能非常耗時，具體取決於您的來源數量。它將清除現有的向量索引並重新為所有內容生成嵌入。",
    startRebuild: "開始重建",
    rebuilding: "正在重建...",
    ...
    rebuild: {
      ...
      howLong: "重建需要多長時間？",
      howLongAns:
        "耗時取決於項目總數、模型速度和 API 速率限制。本地模型（如 Ollama）通常非常快。",
      isSafe: "在使用應用時重建安全嗎？",
      isSafeAns:
  ...
  transformations: {
    ...
    promptHint:
      "提示詞應根據源內容編寫。您可以要求模型總結、提取見解或生成表格等結構化輸出。",
  ...
  models: {
    ...
    missingRequiredModels:
      "缺少必需的模型：{models}。如果没有這些模型，Open Notebook 可能無法正常運行。",
