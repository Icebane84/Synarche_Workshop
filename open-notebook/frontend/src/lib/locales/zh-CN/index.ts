export const zhCN = {
  ...
  sources: {
    ...
    deleteSuccess:
      "来源删除成功。注意：要从存储中删除文件，必须在设置页面中启用“删除文件”选项。",
    ...
    notEmbeddedDesc:
      "此内容尚未为了向量搜索进行嵌入。嵌入可以启用高级搜索功能并更好地发现内容。",
  ...
  searchPage: {
    ...
    noEmbeddingModel:
      "您无法使用此功能，因为尚未选择嵌入模型。请在模型页面设置一个。",
  ...
  podcasts: {
    ...
    workflowHint:
      "单集简介按名称引用发言人简介，因此从发言人开始可以避免以后缺少声音指派。",
    ...
    deleteProfileDesc:
      "这将移除 “{name}”。现有单集将保留其数据，但新单集将不再使用此配置。",
    outlineModel: "大纲模型",
  ...
  advanced: {
    ...
    rebuildWarning:
      "此操作可能非常耗时，具体取决于您的来源数量。它将清除现有的向量索引并重新为所有内容生成嵌入。",
    startRebuild: "开始重建",
    rebuilding: "正在重建...",
    ...
    rebuild: {
      ...
      howLong: "重建需要多长时间？",
      howLongAns:
        "耗时取决于项目总数、模型速度和 API 速率限制。本地模型（如 Ollama）通常非常快。",
      isSafe: "在使用应用时重建安全吗？",
      isSafeAns:
  ...
  transformations: {
    ...
    promptHint:
      "提示词应根据源内容编写。您可以要求模型总结、提取见解或生成表格等结构化输出。",
  ...
  models: {
    ...
    missingRequiredModels:
      "缺少必需的模型：{models}。如果没有这些模型，Open Notebook 可能无法正常运行。",
