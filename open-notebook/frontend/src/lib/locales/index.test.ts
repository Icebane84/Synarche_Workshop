...
describe("Internationalization Locales Integrity", () => {
  ...
  it("zh-CN should have the same keys as en-US", () => {
    ...
    expect(
      missingInZhCN,
      `Missing keys in zh-CN: ${missingInZhCN.join(", ")}`,
    ).toEqual([]);
    expect(
      extraInZhCN,
      `Extra keys in zh-CN: ${extraInZhCN.join(", ")}`,
    ).toEqual([]);
  });
  ...
  it("zh-TW should have the same keys as en-US", () => {
    ...
    expect(
      missingInZhTW,
      `Missing keys in zh-TW: ${missingInZhTW.join(", ")}`,
    ).toEqual([]);
    expect(
      extraInZhTW,
      `Extra keys in zh-TW: ${extraInZhTW.join(", ")}`,
    ).toEqual([]);
  });
  ...
  it("ja-JP should have the same keys as en-US", () => {
    ...
    expect(
      missingInJaJP,
      `Missing keys in ja-JP: ${missingInJaJP.join(", ")}`,
    ).toEqual([]);
    expect(
      extraInJaJP,
      `Extra keys in ja-JP: ${extraInJaJP.join(", ")}`,
    ).toEqual([]);
  });
});
