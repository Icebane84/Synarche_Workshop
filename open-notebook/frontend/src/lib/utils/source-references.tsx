...
export function convertSourceReferences(
  ...
  matches.forEach((match, idx) => {
    ...
    const beforeMatch = text.substring(
      Math.max(0, match.startIndex - 2),
      match.startIndex,
    );
    const hasDoubleBracketBefore = beforeMatch === "[[";
    const hasSingleBracketBefore =
      beforeMatch.endsWith("[") && !hasDoubleBracketBefore;
    ...
    if (textStartIndex < match.startIndex && lastIndex < textStartIndex) {
      ...
      !hasSingleBracketBefore &&
      !hasDoubleBracketBefore
    ) {
    ...
    const afterMatch = text.substring(
      match.endIndex,
      Math.min(text.length, match.endIndex + 2),
    );
    const hasDoubleBracketAfter = afterMatch === "]]";
    const hasSingleBracketAfter =
      afterMatch.startsWith("]") && !hasDoubleBracketAfter;
...
export function convertReferencesToMarkdownLinks(text: string): string {
  ...
  for (let i = references.length - 1; i >= 0; i--) {
    ...
    const contextBefore = result.substring(
      Math.max(0, refStart - 50),
      refStart,
    );
    // Look ahead up to 50 chars for closing brackets/bold markers
    const contextAfter = result.substring(
      refEnd,
      Math.min(result.length, refEnd + 50),
    );
...
export function convertReferencesToCompactMarkdown(
  ...
  for (let i = references.length - 1; i >= 0; i--) {
    ...
    const contextAfter = result.substring(
      refEnd,
      Math.min(result.length, refEnd + 2),
    );
