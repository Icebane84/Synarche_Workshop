...
| Key | Value |
| :--- | :--- |
| **Module ID** | `AOP-ARC-001_ARCHIVALSUPERSESSIONPROTOCOL_V5.0` |
| **Version** | `v11.0` |
...
Stone** entry for the `superseded_artifact_id`. | To modify the central navigation record. | | | 2.2 | **Modify
Record:** Change the status of the old artifact's entry from "Active" to "Superseded." | To officially mark the document
as outdated. | | **3. Archival** | 3.1 | **Move File:** Relocate the file for the `superseded_artifact_id` from its
active directory to the corresponding `_ARCHIVE/` subdirectory. | To preserve the document for historical analysis while
removing it from active use. | | | 3.2 | **Validate Links:** Execute a system query to confirm that the **PRS-001** now
correctly resolves the old ID to the new one. | To verify the re-linking process was successful and prevent broken
navigational paths. |
