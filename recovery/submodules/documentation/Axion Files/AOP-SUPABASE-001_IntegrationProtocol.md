...
- **Edge Functions:** Check Edge Function logs for execution errors.
- **Webhooks:** Check Webhook logs for execution errors.
- **Cron:** Ensure `pg_cron` is enabled. Run:
  `select cron.schedule('axion-discovery', '0 3 * * *', $$ select net.http_post(...) $$);`
- **RLS:** Ensure `Row Level Security` is enabled on all tables to prevent public access. create policies allowing
  access only to authenticated users (The Architect).
