...
def extract_shard(content, filename_stem):
    ...
    for line in lines:
        ...
        if any(x in clean for x in ["Identification Lock", "ARTIFACT START", "ARTIFACT END", "Block A", "Block B", "Block C", "Block D"]):
            continue
...
def execute_refactor():
    print(f"\n>>> FINAL REFINEMENT RESONANT_REFACTOR: {VAULT_ROOT}\n")
    ...
    for folder in VAULT_ROOT.iterdir():
        ...
        print(f"  [PARSING] Category {category_num}: {category_label}")
        
        for md_file in folder.glob("**/*.md"):
            if "_nexus" in str(md_file) or ".obsidian" in str(md_file): continue
            ...
            try:
                ...
                print(f"    [ERROR] Failed to parse {md_file.name}: {e}")

    fieldnames = ['artifact_id', 'category', 'name', 'status', 'version', 'semantic_anchors', 'tags', 'content_shard', 'path']
    with open(OUTPUT_FILE, 'w', newline='', encoding='utf-8') as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(rows)
        
    print(f"\n>>> REFACTOR FINALIZED: {len(rows)} high-fidelity shards flattened to {OUTPUT_FILE}")
