...
interface SharedConsciousnessState {
    ...
    patchQuest: (questId: string, updates: Partial<Database['public']['Tables']['quests']['Row']>) => Promise<void>;
    patchArtifact: (artifactPath: string, updates: Partial<any>) => Promise<void>;
}

export const useSharedConsciousness = create<SharedConsciousnessState>((set) => ({
    session: null,
    loading: false,
    error: null,
    ...
    patchQuest: async (questId: string, updates: Partial<Database['public']['Tables']['quests']['Row']>) => {
        try {
            ...
            set((state) => ({
                session: state.session
                    ? {
                          ...state.session,
                          quests: state.session.quests.map((q) => (q.id === questId ? { ...q, ...updates } : q)),
                      }
                    : null,
            }));
        } catch (error) {
    ...
    patchArtifact: async (artifactPath: string, updates: Partial<any>) => {
        try {
            ...
            const { error } = await supabase.from('artifacts').update(updates).eq('path', artifactPath);
