...
def build_knowledge_graph(workspace_dir: str, output_gv: str) -> None:
    ...
    link_pattern = re.compile(r'\[LINK-(.*?)\]:\s*([^<>-]+?)\s*(<->|->|<-|\\leftrightarrow|\\rightarrow|\\leftarrow)\s*(.*)')
    # Example: GOVERNED_BY: CORE-CODEX-001
    rel_pattern = re.compile(r'(GOVERNED_BY|CONTRIBUTES_TO|UTILIZES|RESONATES|ORCHESTRATES):\s*([A-Za-z0-9_.-]+)')
    
    for root, dirs, files in os.walk(workspace_dir):
        # Exclude entropy
        dirs[:] = [d for d in dirs if d not in (".git", "node_modules", ".venv", "__pycache__", ".agent")]
    ...
    try:
        ...
        with open(output_gv, 'w', encoding='utf-8') as f:
            ...
            f.write('  node [shape=box, style=filled, fillcolor="#2b2d42", fontcolor="#8d99ae", fontname="Courier"];\\n')
            f.write('  edge [color="#ef233c", fontcolor="#edf2f4", fontname="Courier", fontsize=10];\\n')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dir", default=r"c:\\Users\\Chris\\Synarche_Workspace", help="Workspace directory to map")
    parser.add_argument("--out", default=r"c:\\Users\\Chris\\Synarche_Workspace\\axion-core\\lab\\knowledge_graph.gv", help="Output .gv file path")
