...
def register_files():
    ...
    files_to_register = [
        os.path.join(workspace_root, "_governance", "10_Governance", "GVRN.Registry.Origins.md"),
        os.path.join(workspace_root, "_governance", "09_Link", "SYNG.Link.Forge.md"),
        os.path.join(workspace_root, "_governance", "10_Governance", "GVRN.ID.SynthesisInsights.md")
    ]
...
"""
    ...
    # Read the current registry entries to avoid duplicates and append/replace atomically
    # We can split the file by regex of top level keys or read/replace
    # Since GVRN.Master.Registry.yaml is formatted as:
    # KEY:
    #   field: val
    # We can parse the file into keys and their blocks.
    
    entries = {}
    current_key = None
    current_block = []
    
    # We'll split the registry by lines to parse blocks
    if registry_content:
