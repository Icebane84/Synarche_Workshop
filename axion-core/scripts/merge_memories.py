...
def merge():
    ...
    try:
        ...
        for row in src_entries:
            ...
            exists = dst_conn.execute("SELECT id FROM memory_entries WHERE content = ?", (row['content'],)).fetchone()
            if exists:
                id_map[row['id']] = exists['id']
                logger.debug(f"Skipping duplicate memory: {row['id']} -> {exists['id']}")
                continue
            ...
            cursor = dst_conn.execute(
                ...
                (row['content'], row['domain'], row['relevance'], row['confidence'], row['tags'], row['vector'], row['activation_score'], row['state'], row['source'], row['usage_count'], row['memory_layer'], row['last_retrieved'], row['created_at'])
            )
            ...
            if HAS_SQLITE_VEC:
                vec_row = src_conn.execute("SELECT embedding FROM vec_memory_entries WHERE id = ?", (row['id'],)).fetchone()
                if vec_row:
                    dst_conn.execute("INSERT INTO vec_memory_entries (id, embedding) VALUES (?, ?)", (new_id, vec_row['embedding']))

        logger.info(f"Memory entries merged. ID mapping established for {len(id_map)} items.")
        ...
        for row in src_gems:
            if row['entry_id'] in id_map:
                dst_conn.execute(
                    ...
                    (id_map[row['entry_id']], row['insight_label'], row['importance'], row['created_at'])
                )
        ...
        for row in src_assocs:
            if row['source_id'] in id_map and row['target_id'] in id_map:
                dst_conn.execute(
                    ...
                    (id_map[row['source_id']], id_map[row['target_id']], row['relationship_type'], row['strength'], row['created_at'])
                )
        ...
        for row in src_logs:
            dst_conn.execute(
                ...
                (row['timestamp'], row['event_type'], row['module'], row['details'], row['coherence_impact'])
            )
