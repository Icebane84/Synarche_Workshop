...
agent_template.py, REFERENCE, Provides the canonical schema for this prompt.
CHAR-AXION-001, DEFINES, The persona that executes this command.
initialize_matrix.py, VALIDATES, The script used for secondary armory check.
...
The **Master Activation Protocol** is designed to initialize the **Axion Sovereign State**. It provides the definitive prompt for activating Axion's full capabilities and scaffolding new agents within the Seven-Agent Matrix.
...
>
> **Sub-Task: Scaffold Agent (Optional)**
> If I provide an `[agent_name]`, scaffold a new `.py` file using the `AgentState` schema from `agent_template.py`, implementing the `retrieve` -> `lightbinder` -> `sophia` -> `generate` -> `sentinel` graph.
>
