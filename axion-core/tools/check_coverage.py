...
def check_coverage(
    report_path: str, threshold: float, baseline_path: str = None
) -> bool:
    ...
    if coverage_pct < threshold:
        print(
            f"[SOVEREIGNTY VIOLATION] Coverage {coverage_pct}% is below the required minimum of {threshold}%."
        )
        return False

    print(
        f"[PASS] Coverage {coverage_pct}% meets or exceeds the {threshold}% threshold."
    )
    return True
...
if __name__ == "__main__":
    if len(sys.argv) not in [3, 4]:
        print(
            "Usage: python check_coverage.py <report.txt> <threshold> [baseline_report.txt]"
        )
        sys.exit(1)
