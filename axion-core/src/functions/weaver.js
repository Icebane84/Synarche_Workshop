...
export async function weaver(request) {
    try {
        ...
        const baseUrl = Deno.env.get('INSFORGE_BASE_URL') || 'https://x93i6uma.us-east.insforge.app';

        // Extract token from Authorization header or Deno.env
        const authHeader = request.headers.get('Authorization');
        const token = authHeader ? authHeader.replace('Bearer ', '') : Deno.env.get('INSFORGE_API_KEY');

        if (!token) {
            throw new Error('Missing authentication token in Authorization header or environment.');
        }
        ...
        const { error: artifactError } = await insforge.database.from('artifacts').upsert(
            [
                {
                    artifact_id,
        ...
        const { error: memoryError } = await insforge.database.from('memory').insert([
            {
                layer: 2,
