...
class AxionConfig(BaseSettings):
    ...
    # Tool Paths (marked as Optional to avoid Pydantic validation error when None is provided in mock)
    SOPHIA_PATH: Optional[Path] = Field(default=None)
    SENTINEL_PATH: Optional[Path] = Field(default=None)
...
def _resolve_workspace_root() -> Path:
    ...
    for parent in current.parents:
        if (parent / ".agent").exists() or (parent / ".axion-root").exists():
            return parent
