...
class AscensionEngine:
    ...
    LEVEL_THRESHOLDS = {
        "ARCHITECT": {"min_stardust": 10000, "min_authority": 50, "required_specialty": "Logic"},
        "SENTINEL": {"min_stardust": 10000, "min_order": 50, "required_specialty": "Security"},
        "WEAVER": {"min_stardust": 10000, "min_resonance": 50, "required_specialty": "Narrative"}
    }
    ...
    def phase_3_attunement(self, target_class: str) -> Dict[str, Any]:
        ...
        if not self.validate_eligibility(target_class):
            return {"status": ProcessStatus.ERROR, "message": "Ineligible for Ascension."}

        logger.info(f"--- [PHASE 3: ATTUNEMENT] ASCENSION TRIGGERED: {target_class} ---")
        
        # Mutate State
        self.stats["prestige_class"] = target_class
        self.stats["level"] += 1
        self.stats["last_ascension_ts"] = datetime.now().isoformat()
        self.stats["stardust_pool"] = self.stats.get("stardust_pool", 0) + self.stats.get("unprocessed_stardust", 0)
    ...
    def validate_eligibility(self, target_class: str) -> bool:
        ...
        stardust_check = (self.stats.get("stardust", 0) + self.stats.get("unprocessed_stardust", 0)) >= requirements["min_stardust"]
        # Basic check logic
