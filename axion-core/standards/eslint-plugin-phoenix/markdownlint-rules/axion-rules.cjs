...
[
  ...
  if (!CanonicalAcronyms.includes(p)) CanonicalAcronyms.push(p);
});
...
try {
  ...
  DynamicAliasMap = Object.entries(tspaths)
...
const validatepathSecurity = (linkpath, absolutepath, token, onError) => {
  if (!absolutepath.toLowerCase().startsWith(WORKSPACE_ROOT.toLowerCase())) {
    onError({
      lineNumber: token.lineNumber,
...
const validateTsLinkAlias = (token, params, onError) => {
  const AliasMap = DynamicAliasMap;
  const href = token.attrGet("href");
  if (href && (href.endsWith(".ts") || href.endsWith(".tsx"))) {
    if (!href.startsWith("@") && !href.startsWith("http")) {
      let fixedHref = href.replace(/^(\.\.\/|\.\/)+/, "");
      const mapping = AliasMap.find((m) => m.pattern.test(fixedHref));
      ...
      if (exactIndex !== -1) {
        fixInfo = {
          editColumn: exactIndex + lineText.slice(exactIndex).indexOf("(") + 2,
...
module.exports = [
    ...
    function: function PF005(params, onError) {
      params.tokens.forEach((token, index) => {
        const isTargetHeading =
          token.type === "heading_open" && (token.tag === "h2" || token.tag === "h3");
        if (isTargetHeading) {
          // Find heading_close
          ...
          token.children.forEach((child) => {
            if (child.type === "text" && child.content) {
              ...
              while (match !== null) {
                ...
                match = acronymRegex.exec(child.content);
              }
            }
          });
        }
  ...
  },
  {
    ...
    function: function PF011(params, onError) {
      const validIdPrefixes = CanonicalAcronyms;
