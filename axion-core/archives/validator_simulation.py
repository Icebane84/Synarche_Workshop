...
class ValidatorSimulation:
    ...
    def validate(self, artifact_metadata: Dict[str, Any]) -> bool:
        ...
        logger.info(f"Starting simulation for artifact {artifact_metadata.get('Artifact ID', 'Unknown')}...")
        ...
        if self.standard_version not in version and "v15.0" not in version:
            logger.warning(f"FAILED: Version mismatch. Expected {self.standard_version}, got {version}")
            return False
        ...
        logger.info(f"VALIDATION SUCCESS: Artifact {artifact_metadata.get('Artifact ID')} is OMEGA compliant.")
        return True
