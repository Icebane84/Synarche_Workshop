...
def run_carp_cycle(intent: str, output: str, anchors: list):
    ...
    if ccri < 0.85:
        ...
        print("-> Action: Adjust output parameters, enforce Contextual Anchors via AOP-CAM-001.")
        return False
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Cognitive Alignment Refinement Protocol (CARP)")
    parser.add_argument("--intent", required=True, help="Original User Intent (e.g. prompt constraint)")
    parser.add_argument("--output", required=True, help="AI Synthesized Output to verify")
    parser.add_argument("--anchors", nargs="*", default=[], help="Contextual Anchors for Stability check (CSS)")
