...
def execute_safe(cmd: list[str], cwd: str | Path | None = None, **kwargs: Any) -> subprocess.CompletedProcess:
    """
    ...
    for part in cmd:
        if isinstance(part, str) and ("/" in part or "\\" in part or part.endswith(".py") or part.endswith(".ps1") or part.endswith(".sh")):
            ...
            potential_path = Path(cwd or Path.cwd()) / part if not Path(part).is_absolute() else Path(part)
