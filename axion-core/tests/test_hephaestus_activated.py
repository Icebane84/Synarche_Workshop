...
class TestHephaestusActivated:
    def test_soul_narrative_resonance(self):
        ...
        res_neutral = soul.calculate_narrative_resonance(
            "Just a standard test sentence with no special keywords."
        )
        assert res_neutral == pytest.approx(0.5)

        # Test emotional triggers -> should be greater than 0.5
        res_emotional = soul.calculate_narrative_resonance(
            "We achieved victory and triumph, bringing hope to the team!"
        )
    ...
    def test_sentinel_scan_governance(self):
        ...
        test_scan_dir = os.path.abspath(
            os.path.join(current_dir, "..", "src", "hephaestus")
        )
