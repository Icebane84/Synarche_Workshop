...
try:
    ...
    # NOTE: ed25519 import removed for now - AUTHORIZED_PUBLIC_KEYS / ed25519 signing
    # were present in the previous draft but never actually wired into any endpoint.
    # Re-add only once a real verify_ed25519_signature() call path exists; an unused
    # crypto import sitting next to real auth code reads as protection that isn't there.
except ImportError:
    print("[-] Dependency Error: Missing required execution substrates.")
    print("    Execute configuration: pip install fastapi uvicorn jsonschema pydantic")
    sys.exit(1)

# ==========================================
# PHASE I: SYSTEM SCHEMAS & SECRET CONFIG
# ==========================================

app = FastAPI(
    title="The Phoenix Codex Master Substrate",
    version="3.0.1",
    description="Git-for-Meaning framework. HMAC-authenticated writes, schema-validated payloads, "
                 "O(log N) Merkle state anchors, and a crash-isolated single-writer loop."
)

# Fail fast - no insecure default. This was a real regression in v3.0.0 and is
# restored here deliberately: a hardcoded fallback secret defeats the entire
# point of requiring a signature.
ENVIRONMENT_SECRET = os.environ.get("PHOENIX_SECRET")
if not ENVIRONMENT_SECRET:
    print("[-] CRITICAL CONFIGURATION FAULT: PHOENIX_SECRET environment variable is not configured.")
    print("[-] System initialization aborted to prevent insecure fallback operations.")
    sys.exit(1)

SECRET_AUTHENTICATION_KEY = ENVIRONMENT_SECRET.encode('utf-8')
...
# (unchanged - already verified correct)
# ==========================================
...
class IncrementalMerkleEngine:
    """The Mathematical State Anchor. Isolated from semantic intelligence to guarantee pure deterministic scaling."""
        ...
        for level in range(len(self.cached_levels) - 1):
            ...
            if is_left_node:
                if curr_idx + 1 < len(self.cached_levels[level]):
                    ...
                    # Self-duplicate (Bitcoin odd-node convention). Note this proof step
                    # does not distinguish this leaf from a sibling - it's only meaningful
                    # combined with the rest of the path. See review notes.
...
def verify_merkle_proof(leaf_hash_hex: str, proof: list, root_hash_hex: str) -> bool:
    """Independent verifier. Proof is only valid against the root it was generated against -
    if the tree has grown since, regenerate the proof."""
    try:
...
class ImmutableKnowledgeSubstrate:
    def __init__(self, journal_path="phoenix_journal.jsonl", snapshot_path="phoenix_snapshot.json"):
        ...
        # Restored: was dropped in the previous draft, causing /sync-state to 500.
    ...
    def purge_expired_nonces(self, current_runtime_time):
        expired_keys = [k for k, ts in self.seen_nonces.items() if current_runtime_time - ts > 300]
...
def single_writer_processing_loop():
    """The Authoritative Writer Thread.

    Restored: every task is now processed inside its own try/except. In the
    previous draft, one malformed payload (e.g. missing 'timestamp' because
    schema validation had been removed from the endpoint) raised an uncaught
    KeyError here, which killed this daemon thread permanently - the API kept
    returning 202 Accepted for every future proposal while nothing was ever
    actually committed again. A single bad request should never be able to
    take down the whole write path.
    """
    ...
    while True:
        ...
        try:
            ...
            if abs(current_time - p["timestamp"]) > 300:
            ...
            # Defensive floor: log and drop the single bad task, keep the thread alive.
...
def independently_verify_proof(body: ProofVerificationRequest):
    """Fix: previous version took bare leaf_hash_hex/proof/root_hash_hex as plain
    function params, which FastAPI can only interpret as form fields for a list type
    and crashes at import time without python-multipart installed. A proper request
    body model is the correct fix, not adding an unrelated dependency."""
...
async def submit_new_version_proposal(
    request: Request,
    x_phoenix_signature: str = Header(..., description="HMAC-SHA256 signature token.")
):
    """Restored: HMAC verification and schema validation, both removed in the previous
    draft. Without these, any caller (no auth) could write malformed payloads
    (missing required fields) directly into the queue, which is what crashed the
    writer thread - see single_writer_processing_loop."""
    if substrate.stasis_mode:
        raise HTTPException(status_code=503, detail="[-] Write operations disabled. Substrate locked in Stasis Mode.")

    raw_body_bytes = await request.body()

    expected_hmac = hmac.new(SECRET_AUTHENTICATION_KEY, raw_body_bytes, hashlib.sha256).hexdigest()
    if not hmac.compare_digest(x_phoenix_signature, expected_hmac):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,
                             detail="[-] Access Denied: Invalid cryptographic signature footprint.")
