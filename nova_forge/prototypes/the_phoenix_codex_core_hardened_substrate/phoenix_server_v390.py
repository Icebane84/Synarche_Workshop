"""
Phoenix Codex Core Hardened Substrate — Version 3.9.0
======================================================

Patch log (v3.8.0 → v3.9.0):

  FIX-1  : DLQ endpoint was unauthenticated and exposed raw signatures.
            Now requires Ed25519 auth. Stored entries are sanitized
            (signatures stripped, payloads truncated to 512 chars).

  FIX-2  : finalize endpoint had no proposal existence check or
            minimum-stake guard. Added both; also added a configurable
            MIN_FINALIZE_STAKE threshold that must be met before a
            proposal can be closed.

  FIX-3  : OPA context "event_hash" field was set to
            substrate.last_event_hash (the previous event's hash).
            Replaced with content_hash of the incoming request body,
            which is the only correct pre-commit hash available.

  FIX-4  : Fork isolation reused the original proposal's nonce as the
            REGISTER_FORK payload nonce, burning the proposer's nonce.
            Fork events now derive a deterministic but distinct nonce:
            sha256("FORK:" + original_nonce).hexdigest()[:32].

  FIX-5  : historical_ledger grew without bound in memory and was never
            persisted. Replaced with a bounded circular buffer capped at
            LEDGER_MEMORY_LIMIT events, queryable via a paginated endpoint.

  FIX-6  : ProofVerificationRequest model existed since v3.5.0 with no
            endpoint. Added POST /api/v1/merkle/verify and
            GET  /api/v1/merkle/proof/{leaf_index} to close the gap.

  FIX-7  : GET /api/v1/governance/proposals had no single-proposal lookup.
            Added GET /api/v1/governance/proposals/{proposal_id}.

  FIX-8  : GET /api/v1/health was absent; operators had no observability
            surface. Added a health endpoint returning substrate vitals.

  FIX-9  : DLQ grew without bound. Added DLQ_MAX_SIZE cap with oldest-
            entry eviction to prevent unbounded memory growth.

  FIX-10 : substrate.last_event_hash read from HTTP handler while the
            writer thread may update it concurrently. Protected all
            cross-thread substrate reads in HTTP handlers with a
            dedicated read lock.

Outstanding (tracked but not yet resolved):
  - OPEN-1: COMMIT_VOTE has no stake-cap per node (unlimited vote stuffing).
  - OPEN-2: No voting period timeout; proposals stay in VOTING indefinitely.
  - OPEN-3: Fork tie-breaker compares node_origin against causal_trigger
            (a description string) lexicographically — semantically
            confused. Needs a canonical node-priority registry.
"""
...
try:
    ...
    print("    pip install fastapi uvicorn jsonschema pydantic cryptography")
    sys.exit(1)


# ==========================================
# PHASE I: CONFIGURATION & SCHEMAS
# ==========================================

app = FastAPI(
    title="Phoenix Codex Core Hardened Substrate",
    version="3.9.0",
    description=(
        "Git-for-Meaning framework: non-blocking OPA gates, "
        "authenticated DLQ, bounded ledger, Merkle proof API, "
        "stake-gated finalization."
    ),
)
...
# ── PKI ───────────────────────────────────────────────────────────────────────
AUTHORIZED_PUBLIC_KEYS: dict[str, str] = {
...
if _PLACEHOLDER in AUTHORIZED_PUBLIC_KEYS.values():
    print("[-] CRITICAL: AUTHORIZED_PUBLIC_KEYS contains placeholder values.")
    sys.exit(1)

# ── Governance parameters ─────────────────────────────────────────────────────
REPLAY_WINDOW_SECONDS: int   = 300
MIN_FINALIZE_STAKE:    float = 1.0   # minimum total support_stake to close a proposal
DLQ_MAX_SIZE:          int   = 10_000
LEDGER_MEMORY_LIMIT:   int   = 50_000  # max in-memory historical events

# ── Intent-attribution guard ──────────────────────────────────────────────────
...
class IncrementalMerkleEngine:
    ...
    def _recalculate_right_edge(self) -> bytes:
        ...
        while True:
            single_root = (
                curr_level == len(self.cached_levels) - 1
                and len(self.cached_levels[curr_level]) == 1
            )
            if single_root:
            ...
            if is_left:
                ...
                right = left  # odd-length duplication
            else:
    ...
    def generate_merkle_proof(self, leaf_index: int) -> list[tuple[int, str]]:
...
class SemanticEntryGraph:
    def __init__(self):
        ...
        self.refs:    dict[str, str]  = {"main": "ROOT_GENESIS"}
...
def _sanitize_dlq_entry(entry: dict) -> dict:
    """
    FIX-1: Strip raw signatures from DLQ entries before storage.
    Keeps enough for debugging without leaking auth material.
    """
    safe = {
    ...
    if isinstance(raw, dict):
        # Keep structural metadata, drop full assertion content
...
class ImmutableKnowledgeSubstrate:
    def __init__(self, journal_path="phoenix_journal.jsonl", snapshot_path="phoenix_snapshot.json"):
        ...
        self.node_registry:    dict[str, SemanticEntryGraph] = {}
        self.proposal_registry: dict[int, dict]              = {}
        self.seen_nonces:       dict[str, int]               = {}
        self.quarantined_peers: set[str]                     = set()

        self._stasis_event = threading.Event()

        # FIX-5: Bounded circular ledger — no more unbounded list growth
        self._ledger: deque[SystemCausalEvent] = deque(maxlen=LEDGER_MEMORY_LIMIT)

        self.aistf_transactions: list[dict] = []
        self.dead_letter_queue:  list[dict] = []  # FIX-9: capped at DLQ_MAX_SIZE on append
        ...
        # FIX-10: Protects cross-thread reads of last_event_hash / total_blocks_processed
    ...
    def set_stasis_lockdown(self) -> None:
        self._stasis_event.set()

    # ── DLQ append with size cap ──────────────────────────────────────────────
    def append_dlq(self, entry: dict) -> None:
        """FIX-1 + FIX-9: sanitize before storage; evict oldest when cap reached."""
    ...
    # ── Core commit ───────────────────────────────────────────────────────────
    def commit_validated_event(self, event: SystemCausalEvent, recover_mode: bool = False) -> bool:
        ...
        # ── Shared tail: ledger, nonce, Merkle, counter ───────────────────────
    ...
    def generate_compacted_state_snapshot(self) -> bool:
        try:
            ...
            with open(self.snapshot_path, "w", encoding="utf-8") as sf:
                json.dump(
                    {"manifest_integrity_hash": snap_hash, "snapshot_payload": snapshot_data},
            ...
            with open(self.journal_path, "r", encoding="utf-8") as jf:
                for raw_line in jf:
                    ...
                    if d["action_type"] == "SYSTEM_STASIS":
                        self.set_stasis_lockdown()
                        continue
                    if d["event_id"] <= self.total_blocks_processed:
                        continue
                    ...
                    if ev._compute_hash() != d["event_hash"]:
                        print("[!] Journal hash mismatch — aborting recovery.")
                        sys.exit(1)
    ...
    # ── Read helpers (hold lock for consistent multi-field snapshot) ──────────
    def read_state_snapshot(self) -> dict:
        """FIX-10: Atomic multi-field read for HTTP handlers."""
        with self._state_read_lock:
            return {
                "last_event_hash":       self.last_event_hash,
                "global_state_root":     self.global_state_root,
...
tx_queue:  queue.Queue = queue.Queue(maxsize=10_000)
...
def _derive_fork_nonce(original_nonce: str) -> str:
    """
    FIX-4: Produce a deterministic but distinct nonce for REGISTER_FORK
    events so the original proposer's nonce is not consumed.
    """
    return hashlib.sha256(f"FORK:{original_nonce}".encode()).hexdigest()[:32]


def single_writer_processing_loop() -> None:
    """Authoritative single-writer thread. All substrate mutations happen here."""
    global tx_queue, substrate, active_sockets

    while True:
        try:
            hash_task = tx_queue.get(timeout=1.0)
        except queue.Empty:
            continue
        ...
        try:
            ...
            if abs(current_time - p.get("timestamp", 0)) > REPLAY_WINDOW_SECONDS:
                continue
            if p.get("nonce") in substrate.seen_nonces:
                continue
            if substrate.is_stasis_active():
                continue

            # ── Fork-resolution gate ─────────────────────────────────────────
                    ...
                    if declared_parent != current_tip_hash:
                        ...
                        try:
                            tip_ts = datetime.fromisoformat(
                                tip_block["compiled_timestamp"].replace("Z", "+00:00")
                            ).timestamp()
                        except Exception:
                            tip_ts = 0.0
                        ...
                        if incoming_ts > tip_ts or (
                            incoming_ts == tip_ts
                            and node_origin > tip_block.get("causal_trigger", "")
                        ):
                            ...
                            # FIX-4: distinct nonce so original proposer's nonce is preserved
                            ...
                            fork_payload = {
                                ...
                                "nonce":       fork_nonce,   # FIX-4
                            }
                            ev = SystemCausalEvent(
                                fork_id, "REGISTER_FORK", fork_payload,
                                node_id=node_origin, timestamp=p["timestamp"]
                            )
                            # FIX-D (from v3.8) + FIX-C (gossip forks)
                            if substrate.commit_validated_event(ev):
                                gossip = json.dumps({
                                    "action_type": "REGISTER_FORK",
                                    "payload":     fork_payload,
                                    "node_id":     node_origin,
                                })
                                with _sockets_lock:
                                    snap = set(active_sockets)
                                for sock in snap:
                                    _safe_broadcast(sock, gossip)
                            continue

                        elif incoming_ts < tip_ts:
                            print("[-] Stale Fork Rejected: payload predates active tip.")
                            continue
                        else:  # equal timestamp, lost tie-breaker
                            print("[-] Branch Isolated: lost lexicographic tie-breaker.")
                            continue
            ...
            if substrate.commit_validated_event(ev):   # FIX-D
...
def _safe_broadcast(sock, message: str) -> None:
...
async def bootstrap_operating_substrate() -> None:
    ...
    print("[+] Phoenix Codex v3.9.0 — substrate active.")
...
def health_check():
    """
    FIX-8: Observability surface — returns substrate vitals without
    ...
    return {
        ...
        "active_proposals":     sum(
            1 for p in substrate.proposal_registry.values()
            if p["status"] == "VOTING"
        ),
...
def get_merkle_proof(leaf_index: int):
    """
    FIX-6: Expose the Merkle proof API that ProofVerificationRequest implied
    but no endpoint served since v3.5.0.
    """
...
def verify_proof_endpoint(req: ProofVerificationRequest):
    """
    FIX-6: Endpoint that finally puts ProofVerificationRequest to use.
    """
...
async def websocket_p2p_sync(websocket: WebSocket, x_phoenix_node_id: str = Header(...)):
    ...
    print(f"[+] P2P connected: {x_phoenix_node_id}")
    ...
    try:
        while True:
            ...
            if sender_id in substrate.quarantined_peers:
                continue
            if payload.get("nonce") in substrate.seen_nonces:
                continue
            try:
                tx_queue.put_nowait({"action_type": action_type, "payload": payload, "node_id": sender_id})
            except queue.Full:
                print(f"[-] Backpressure: dropped packet from {sender_id}")
    except WebSocketDisconnect:
        print(f"[-] P2P disconnected: {x_phoenix_node_id}")
    finally:
        with _sockets_lock:
...
def list_proposals(status_filter: str | None = None):
    """List all proposals, optionally filtered by status (VOTING/RATIFIED/REJECTED)."""
...
async def submit_proposal(
    request: Request,
    x_phoenix_node_id: str = Header(...),
    x_phoenix_signature: str = Header(...),
):
    if substrate.is_stasis_active():
        raise HTTPException(503, "[-] Substrate locked in Stasis Mode.")
    ...
    # FIX-3 + FIX-10: atomic state read; correct event_hash = content_hash, not last_event_hash
    state        = substrate.read_state_snapshot()
    ...
    if entry_id in substrate.node_registry:
        graph = substrate.node_registry[entry_id]
        relevant_commits = [
            {"event_hash": h, "prev_hash": b.get("parent_hash")}
            for h, b in graph.commits.items()
        ]
    ...
    opa_input = {
        "input": {
            ...
            "chronicle": {
                ...
                "latest_hash":  state["last_event_hash"],    # FIX-10: consistent read
                "events":       relevant_commits,
            },
            "event": {
                "id":           f"evt_{payload['nonce'][:8]}",
                "artifact_id":  "CORE.CODEX.PhoenixSchema",
                "law_id":       "LAW-035",
                "type":         "APPEND",
                "timestamp":    datetime.utcfromtimestamp(payload["timestamp"]).isoformat() + "Z",
                "prev_hash":    payload["lineage"]["parent_hash"],
                "content_hash": content_hash,
                "event_hash":   content_hash,  # FIX-3: was last_event_hash (wrong)
                "payload":      payload,
                "signatures":   [{"role": "ARCHITECT", "signature": x_phoenix_signature}],
            },
        }
    }

    # Non-blocking OPA evaluation with isolated temp file (FIX-1 + FIX-2 from v3.7)
    ...
    try:
        ...
        cmd    = ["opa", "eval", "-d", "chronicle_integrity.rego", "-i", tmp_path,
                  "data.phoenix.chronicle.integrity.allow"]
        ...
        # OPA binary absent — fail closed, log clearly
        print("[!] OPA binary not found. All proposals denied until OPA is installed.")
    except Exception as exc:
        print(f"[!] OPA evaluation failed: {exc}")
    finally:
    ...
    if not allow_decision:
        substrate.append_dlq({
            ...
            "timestamp_captured": time.time(),
            "raw_payload":    payload,
            "audit_meta":     {"node_origin": x_phoenix_node_id, "reason": "chronicle_integrity.rego denied."},
        })
    ...
    try:
        ...
        raise HTTPException(429, "[-] Queue overflow.")
...
async def cast_vote(
    ...
    if substrate.is_stasis_active():
        raise HTTPException(503, "[-] Stasis active.")
    raw_body = await request.body()
    payload  = verify_signature_and_freshness(x_phoenix_node_id, x_phoenix_signature, raw_body)
    try:
        Draft202012Validator(PHOENIX_VOTE_SCHEMA).validate(instance=payload)
    except ValidationError as err:
        raise HTTPException(400, f"[-] Vote schema error: {err.message}")
    try:
        tx_queue.put_nowait({"action_type": "COMMIT_VOTE", "payload": payload, "node_id": x_phoenix_node_id})
    except queue.Full:
        raise HTTPException(429, "[-] Queue overflow.")
...
async def finalize_proposal(
    ...
    """
    FIX-2: Added proposal existence check and minimum stake guard.
    Any authenticated node may trigger finalization, but the proposal
    must exist, be in VOTING status, and meet MIN_FINALIZE_STAKE.
    """
    if substrate.is_stasis_active():
        raise HTTPException(503, "[-] Stasis active.")
    raw_body = await request.body()
    ...
    # FIX-2a: existence check
    prop = substrate.proposal_registry.get(proposal_id)
    if not prop:
        raise HTTPException(404, f"[-] Proposal {proposal_id} does not exist.")
    if prop["status"] != "VOTING":
        raise HTTPException(409, f"[-] Proposal {proposal_id} is not in VOTING state (current: {prop['status']}).")

    # FIX-2b: minimum stake guard — prevents premature finalization with no votes
    total_stake = prop["support_stake"] + prop["against_stake"]
    if total_stake < MIN_FINALIZE_STAKE:
        raise HTTPException(
            422,
            f"[-] Insufficient stake to finalize: {total_stake:.2f} < {MIN_FINALIZE_STAKE:.2f} required."
        )

    try:
        tx_queue.put_nowait({
            "action_type": "FINALIZE_PROPOSAL",
            "payload":     {"proposal_id": proposal_id, "timestamp": payload["timestamp"], "nonce": payload["nonce"]},
            "node_id":     x_phoenix_node_id,
        })
    except queue.Full:
        raise HTTPException(429, "[-] Queue overflow.")
...
def get_dlq(
    ...
    """
    FIX-1: DLQ endpoint now requires Ed25519 authentication.
    Returns sanitized entries (signatures and full assertion text stripped).
    """
    # Minimal auth-only check (no body needed for a GET)
    if x_phoenix_node_id not in AUTHORIZED_PUBLIC_KEYS:
        raise HTTPException(status.HTTP_401_UNAUTHORIZED, "[-] Unregistered node identity.")
    # Signature over the empty body for GET requests — clients sign b"" with their key
    try:
        pub = bytes.fromhex(AUTHORIZED_PUBLIC_KEYS[x_phoenix_node_id])
        sig = bytes.fromhex(x_phoenix_signature)
        ed25519.Ed25519PublicKey.from_public_bytes(pub).verify(sig, b"")
    except Exception:
        raise HTTPException(status.HTTP_401_UNAUTHORIZED, "[-] Signature mismatch.")
...
def get_ledger_tail(
    ...
    """
    FIX-5: Paginated tail of the bounded in-memory ledger. Authenticated.
    """
    if x_phoenix_node_id not in AUTHORIZED_PUBLIC_KEYS:
        raise HTTPException(status.HTTP_401_UNAUTHORIZED, "[-] Unregistered node identity.")
    try:
