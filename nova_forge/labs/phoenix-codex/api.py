...
async def websocket_p2p_sync(websocket: WebSocket, x_phoenix_node_id: str = Header(...)):
    ...
    async with worker.get_sockets_lock(): worker.active_sockets.add(websocket)
    try:
        while True:
        ...
        async with worker.get_sockets_lock(): worker.active_sockets.discard(websocket)
...
async def submit_proposal(request: Request, x_phoenix_node_id: str = Header(...), x_phoenix_signature: str = Header(...)) -> dict:
    if not worker.substrate: raise HTTPException(503, "Substrate not initialized.")
