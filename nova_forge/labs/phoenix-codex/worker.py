...
# NOTE: asyncio.Lock() must NOT be created at module level — it would bind to no loop
# and raise RuntimeError when awaited. Initialized lazily on first use in bootstrap.
_sockets_lock: asyncio.Lock | None = None
MAIN_EVENT_LOOP: asyncio.AbstractEventLoop | None = None


def get_sockets_lock() -> asyncio.Lock:
    """Lazy initializer — always called from within an async context."""
    global _sockets_lock
    if _sockets_lock is None:
        _sockets_lock = asyncio.Lock()
    return _sockets_lock
        ...
        if substrate.commit_validated_event(ev):
            ...
            if MAIN_EVENT_LOOP and MAIN_EVENT_LOOP.is_running():
                MAIN_EVENT_LOOP.call_soon_threadsafe(
                    lambda g=gossip: asyncio.ensure_future(broadcast_to_peers(g), loop=MAIN_EVENT_LOOP)
                )
            else:
                print("[!] FORK gossip skipped: main event loop not available.")
    else:
...
async def broadcast_to_peers(message: str):
    ...
    Must be called from within the main event loop (via call_soon_threadsafe
    from worker threads, or directly from async endpoints).
    """
            ...
            if substrate.commit_validated_event(ev):
                if node_origin == "LOCAL":
                    ...
                    if MAIN_EVENT_LOOP and MAIN_EVENT_LOOP.is_running():
                        ...
                        print(f"[!] Gossip skipped for '{action_type}': main event loop not available.")
