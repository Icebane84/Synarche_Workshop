...
export class GraphCombatEngine {
  ...
  private tick() {
    ...
    // 1. Garrett Attacks (Check for Alerion Reflexes talent)
    let exposedPerHit = 1;
    ...
    // 2. Serafina projects shield (Check for Warden's Grace)
    ...
    // Check if Kaelen has Eternal Ember talent for passive regen
    ...
    // 3. Kaelen Attacks (Inspects WIELDS link to Relic & L1 Gem Crit Boost)
    ...
    // OMEGA Layer Combat Abilities: Check for L1 Gems crit boost
    const isGemCrit = Math.random() < 0.35; // 35% Gem Crit chance
    ...
    // 4. Enemy attacks (if not stunned)
