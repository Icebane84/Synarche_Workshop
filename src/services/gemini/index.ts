...
export const queryCognitiveCore = async (
    ...
    const finalSystemInstruction = baseInstruction + contextInstruction + pastChatsInstruction + ragInstruction;

    // Map history to Ollama / Gemini formats
    const ollamaHistory = history.map((h) => ({
        role: (h.sender === 'user' ? 'user' : 'assistant') as 'user' | 'assistant',
        content: h.text,
    }));

    // Use local Ollama (zero tokens, 100% sovereign) if requested or if Gemini Key is absent
    const useLocal = systemConfig.api.provider === 'ollama' || (!systemConfig.api.geminiKey && systemConfig.api.provider === 'auto');
    if (useLocal) {
        try {
            return await queryOllamaCore(prompt, finalSystemInstruction + NEURAL_LINK_INSTRUCTION, ollamaHistory);
        } catch (err: unknown) {
