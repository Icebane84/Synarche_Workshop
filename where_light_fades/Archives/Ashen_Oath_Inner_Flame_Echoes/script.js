...
const gameConfig = {
  ...
  structures: {
    sanctum: {
      name: "Hallowed Chapel",
      desc: "A place of Order. Generates Faith.",
      baseCost: { faith: 10 },
      output: { faith: 1 },
      resonancePull: 1,
      costScaling: 1.5,
    ...
    spire: {
      name: "Obsidian Shard",
      desc: "A conduit for Nyx. Generates Doubt.",
      baseCost: { doubt: 10 },
      output: { doubt: 1 },
      resonancePull: -1,
      sanityPenalty: 0.1,
    ...
    regimen: {
      name: "Sentinel's Drill",
      desc: "Garrett's training methods. Generates Resolve.",
      baseCost: { faith: 5, doubt: 5 },
      output: { resolve: 0.5 },
      costScaling: 1.5,
    },
    censer: {
      name: "Serafina's Ward",
      desc: "Soothing incense. Restores Sanity.",
      baseCost: { faith: 100, resolve: 20 },
      output: { sanity: 0.1 },
      costScaling: 1.5,
    },
  },
  ...
  researchData: {
    ...
    voidTouched: {
      ...
      desc: "Oathbringer generates +1 resource per click for every 10 Obsidian Shards.",
      cost: { faith: 150, doubt: 150 },
    },
    litanyLost: {
      name: "Litany of the Lost",
      desc: "Hallowed Chapels produce +0.5 Resolve/sec.",
      cost: { faith: 300 },
    },
    cognitiveDissonance: {
      name: "Cognitive Dissonance",
      desc: "+1% Doubt gen for every 1% missing Sanity.",
      cost: { doubt: 500 },
    },
    serafinaTincture: {
      name: "Serafina's Tinctures",
      desc: "Serafina's Wards are 2x effective.",
      cost: { faith: 200, resolve: 200 },
    },
    watcherEye: {
      name: "The Watcher's Eye",
      desc: "10% chance for 5x Critical Clicks.",
      cost: { resolve: 400 },
    },
    archGeometry: {
      name: "Architectural Geometry",
      desc: "Reduces Chapel cost scaling to 1.4.",
      cost: { faith: 1000 },
    },
    shadowBargain: {
      name: "Shadow Bargain",
      desc: "Unlocks Dark Mode Toggle (Triple production, 5x Sanity Drain).",
      cost: { doubt: 1000 },
    },
    leylineMapping: {
      name: "Leyline Mapping",
      desc: "Heartstone Rituals fill 20% faster.",
      cost: { faith: 750, doubt: 750 },
    },
    eldrinRegret: {
      name: "Eldrin's Regret",
      desc: "Obsidian Shards produce Faith instead of Doubt (50% rate).",
      cost: { resolve: 2000 },
    },
    ascendantEcho: {
      name: "The Ascendant's Echo",
      desc: "Retain 10% of structures after Ascension.",
      cost: { faith: 5000, doubt: 5000, resolve: 5000 },
    },
  },

  prestigeUpgrades: {
    ancestralMemory: {
      name: "Eldrin's Memory",
    ...
    mentalResilience: {
      ...
      baseCost: 2,
      costScaling: 2,
      effect: (l) => Math.max(0.1, 1 - l * 0.05),
    },
    lumenMastery: {
  ...
  eventData: [
    {
      ...
      choices: [
        {
          text: "Burn it.",
          costLabel: "-10 Resolve, +5 Sanity",
          effect: (gs) => {
            gs.resources.resolve -= 10;
            gs.sanity += 5;
            return "It burns with a sickly green flame.";
          },
        },
        {
          text: "Keep it.",
          costLabel: "+30 Doubt, -10 Sanity",
          effect: (gs) => {
            gs.resources.doubt += 30;
            gs.sanity -= 10;
            return "Oathbringer hums in approval.";
          },
        },
      ],
    },
...
function toggleDarkMode() {
  ...
  logEvent(
    gameState.darkModeActive
      ? "You embrace the shadow."
      : "You suppress the darkness.",
    "system",
  );
}
...
function checkHeartstoneUnlock() {
  if (
    !gameState.heartstone.unlocked &&
    gameState.lifetime.faith > gameConfig.heartstoneConfig.unlockThreshold
  ) {
...
function channelHeartstone(type) {
  ...
  if (type === "faith") {
    if (gameState.resources.faith >= costFaith) {
      ...
      gameState.heartstone.purification +=
        gameConfig.heartstoneConfig.purifyAmt * mult;
      spawnFloatingText(
        window.innerWidth / 2,
    ...
    if (gameState.resources.resolve >= costResolve) {
      ...
      gameState.heartstone.purification +=
        gameConfig.heartstoneConfig.purifyAmt * 1.5 * mult;
      spawnFloatingText(
        window.innerWidth / 2,
        window.innerHeight / 2,
        "STRIKE!",
...
function endHeartstone(victory) {
  ...
  if (victory) {
    ...
    logEvent(
      `Heartstone Purified! Level ${gameState.heartstone.level} Reached.`,
      "good",
    );
...
function calculateProduction() {
  ...
  if (gameState.research["eldrinRegret"]) {
    ...
    prod.doubt +=
      gameState.structures.spire * gameConfig.structures.spire.output.doubt;
  }

  if (gameState.structures.regimen > 0) {
    prod.resolve +=
      gameState.structures.regimen *
...
function gameTick() {
  ...
  try {
    ...
    if (typeof audio !== "undefined")
      audio.updateAtmosphere(gameState.sanity, gameState.resonance);
...
function updateUI() {
  ...
  document.getElementById("res-doubt").innerText = Math.floor(
    gameState.resources.doubt,
  );
  document.getElementById("res-resolve").innerText = Math.floor(
    gameState.resources.resolve,
  );
  ...
  if (sanityBar) {
    ...
    if (gameState.sanity > 50)
      sanityBar.style.backgroundColor = "var(--col-sanity)";
  ...
  if (gameState.heartstone.unlocked) {
    ...
    const purPct = Math.min(
      100,
      (gameState.heartstone.purification / maxIntegrity) * 100,
    );
    const corPct = Math.min(
      100,
...
function attemptFusion() {
  if (
    gameState.resources.faith >= 5000 &&
    gameState.resources.doubt >= 5000 &&
    gameState.resources.resolve >= 5000
  ) {
    gameState.gameWon = true;
    logEvent("KAELEN AND ELDRIN FUSE. THE CYCLE IS BROKEN.", "ending");
    alert(
      "YOU HAVE ACHIEVED THE TRUE ENDING. Kaelen and Eldrin are one. The Shadow Self is bound. The world begins to heal.",
    );
...
function checkUnlocks() {
  ...
  const totalLifetime =
    gameState.lifetime.faith +
    gameState.lifetime.doubt +
    gameState.lifetime.resolve;
  const ascensionBtn = document.getElementById("btn-tab-ascension");
  const shouldShowAscension =
    totalLifetime >= gameConfig.prestigeUnlockThreshold ||
    gameState.prestige.timesAscended > 0;
  if (shouldShowAscension && ascensionBtn.style.display === "none") {
    ascensionBtn.style.display = "inline-block";
    if (gameState.prestige.timesAscended === 0)
      logEvent("A path to Ascension reveals itself...", "prestige");
  }
}
...
function switchTab(tabName) {
  ...
  if (tabName === "ascension")
    document.getElementById("btn-tab-ascension").classList.add("active");
...
function loadGame() {
  ...
  if (savedGame) {
    ...
    if (parsedGame.resources)
      gameState.resources = { ...gameState.resources, ...parsedGame.resources };
    if (parsedGame.structures)
      gameState.structures = {
        ...gameState.structures,
        ...parsedGame.structures,
      };
    if (parsedGame.heartstone)
      gameState.heartstone = {
        ...gameState.heartstone,
        ...parsedGame.heartstone,
      };
    if (parsedGame.prestige)
      gameState.prestige = { ...gameState.prestige, ...parsedGame.prestige };
  }
  checkUnlocks();
  checkTrueEndingConditions();
  switchTab("sanctum");
