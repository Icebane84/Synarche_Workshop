...
class NovaGenesis {
  ...
  validateArtifactId() {
    ...
    if (!id || typeof id !== "string") {
      return {
        valid: false,
        errors: ["Artifact ID must be a non-empty string."],
      };
    }
    ...
    if (parts.length < 2) {
      errors.push(
        `RNC_VIOLATION: '${id}' must follow DOMAIN.Subject[.Type] (min 2 segments).`,
      );
    }

    if (!VALID_DOMAINS.has(parts[0])) {
      errors.push(
        `UNKNOWN_DOMAIN: '${parts[0]}' not in OGLN registry. Valid: ${[...VALID_DOMAINS].join(", ")}`,
      );
    }
