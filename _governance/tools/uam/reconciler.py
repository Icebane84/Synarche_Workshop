...
class Reconciler:
    ...
    @staticmethod
    def prompt_interactive_reconciliation(
        file_path: str, diagnostics: list[dict]
    ) -> bool:
        ...
        for diag in diagnostics:
            icon = (
                ...
                else ("⚠️" if diag["severity"] == "WARNING" else "ℹ️")
            )
            print(f"  {icon} {diag['severity']}: {diag['msg']}")

        try:
            choice = (
                input(f"Apply standardizations to '{filename}'? [y/N]: ")
                .strip()
                .lower()
            )
            return choice == "y"
