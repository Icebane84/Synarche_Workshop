    ...
    try:
        ...
        for line in lines:
            if line.startswith("#"):
                ...
                if "Universal Identification" in clean:
                    continue
                if "Genesis Stamp" in clean:
                    continue
                if re.match(r"\**Tab \d+\**", clean, re.IGNORECASE):
                    continue
    ...
    for subdir in SUBDIRS:
        ...
        for filename in os.listdir(dir_path):
            if filename.endswith(".md"):
                ...
                results.append({"subdir": subdir, "filename": filename, "title": title})

    # Output as a markdown table for easy reading by the LLM
    print("| Subdirectory | Filename | Actual Title |")
    print("| :--- | :--- | :--- |")
    for r in results:
