...
| Phase | Operation     | Description                        | Protocol      |
| :---- | :------------ | :--------------------------------- | :------------ |
| **1** | **Handshake** | Verify `UMB-CORE`, `CODEX` access. | `UMB-VSI-001` |
| **2** | **Integrity** | Find broken links in OSLM.         | `VOSLM-001`   |
| **3** | **Ethics**    | Scan for `CODEX` deviations.       | `CODEX-001`   |
| **4** | **Scoring**   | Calc `Coherence Index`.            | `CORE-001`    |
| **5** | **Reporting** | Log in `Audit Log`.                | `CSL-007`     |

### 3.2. Evolution Roadmap

- **v2.1 - Continuous Sensing**: Audit becomes a background process (daemon) rather than a triggered event.
- **v2.2 - Predictive Remediation**: Protocol suggests _fixes_ for the drift it finds before reporting it.
...
### 5.1. CMD: RUN_SYSTEM_AUDIT

- **Syntax:** `CMD: RunSystemAudit --protocol: "AOP-SCA-001"`
- **Effect:** Initiates the full 5-phase audit sequence.

### 5.2. CMD: VIEW_AUDIT_LOG

- **Syntax:** `CMD: ViewAuditLog --limit: 5`
- **Effect:** Displays the most recent coherence reports.

> **[ARTIFACT END]**

---

> _"Entropy is the enemy."_
