...
class SovereignRPGCrafter(PhoenixBase):
    ...
    def __init__(self, verbose: bool = False):
        super().__init__(persona_id="Sophia_Crafter", ethos="Architectural Precision", verbose=verbose)
    ...
    def _Parse_Celestial_Tier(self, Value_String: str) -> int:
        ...
        if "Star Tier" in Value_String: return 10
        if "Nova Tier" in Value_String: return 20
    ...
    def _Extract_Sovereign_Artifact(self, File_Path: str) -> Optional[Dict[str, Any]]:
        ...
        if "BLK-RPG-001" not in Content: return None

        # Identification Extraction
        Artifact_ID = (re.search(r"\*\*Artifact ID\*\*\s*\|\s*`?(.*?)`?\s*\|", Content) or [None, "Unknown"])[1].strip()
        
        # System Slot & Resource Economics
        Raw_Slot = (re.search(r"\*\*System Slot:\*\*\s*`\[(.*?)\|?.*?\]`", Content) or [None, "Utility"])[1].strip()
        Cognitive_Load = (re.search(r"\*\*Cognitive Load Draw:\*\*\s*`\[(.*?)\]`", Content) or [None, "Low"])[1].strip()
        ...
        if Primary_Stat and Stat_Value_Raw:
            Axiom_Manifest[Primary_Stat.group(1)] = self._Parse_Celestial_Tier(Stat_Value_Raw.group(1))
        ...
        return {
            ...
            "rarity_rationale": (re.search(r"_Rarity Rationale:_\s*`\[(.*?)\]`", Content) or [None, "N/A"])[1].strip()
        }
