  ...
  const characterDocs: KnowledgeDocument[] = nodesList
    ...
    .map((n) => {
      ...
      const connectedEdges = edgesList
        ...
        .slice(0, 10)
        .map((e) => {
          const notesStr = e.properties?.notes ? '(' + String(e.properties.notes) + ')' : '';
          return `- ${e.source} --[${e.relation}]--> ${e.target} ${notesStr}`.trimEnd();
        })
        .join('\n');
...
`,
  ...
  docs.push(worldDoc, ...characterDocs.slice(0, 50)); // Include top 50 detailed character & item nodes
