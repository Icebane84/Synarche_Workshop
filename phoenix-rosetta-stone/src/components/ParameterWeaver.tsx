...
interface ParameterWeaverProps {
    ...
    onSubmit: (params: Record<string, WeaverValue>) => void;
    onCancel: () => void;
}

const ParameterWeaver: React.FC<ParameterWeaverProps> = ({ command, onSubmit, onCancel }) => {
    const [values, setValues] = useState<Record<string, WeaverValue>>(() => {
        const initial: Record<string, WeaverValue> = {};
    ...
    const handleValueChange = (paramName: string, value: WeaverValue) => {
        setValues((prev) => ({ ...prev, [paramName]: value }));
    };

    const isFormValid = command.parameters.every((param) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (isFormValid) {
            ...
            const finalValues: Record<string, WeaverValue> = { ...values };
            command.parameters.forEach((p) => {
                const val = finalValues[p.name];
                if (p.type === 'string[]' && typeof val === 'string') {
                    finalValues[p.name] = val
                        .split(',')
                        .map((s: string) => s.trim())
                        .filter((s: string) => s !== '');
                }
    ...
    const renderInput = (param: CommandDefinitionGUCAv5['parameters'][0]) => {
        ...
        if (param.uiHint === 'artifact') {
            ...
            if (command.commandId === 'CMD_SIMULATE_SYNERGY') {
                ...
                const otherParam = command.parameters.find((p) => p.uiHint === 'artifact' && p.name !== param.name);
                if (otherParam) {
                    disabledId = values[otherParam.name] as string | null;
                }
            }
            return (
                <ArtifactSelector
                    artifacts={graphData.nodes}
                    selectedId={(values[param.name] as string | null) ?? null}
                    onSelect={(id) => {
                        handleValueChange(param.name, id);
                    }}
        ...
        if (param.uiHint === 'textarea') {
            return (
                <textarea
                    value={(values[param.name] as string | undefined) ?? ''}
                    onChange={(e) => {
                ...
                return (
                    <div className="flex items-center">
                        <input
                            ...
                            onChange={(e) => {
                                handleValueChange(param.name, e.target.checked);
                            }}
            ...
            case 'number':
                return (
                    <input
                        ...
                        onChange={(e) => {
                            handleValueChange(param.name, e.target.valueAsNumber);
                        }}
    ...
    return (
        <div className="p-4 md:p-8 h-full flex flex-col w-full max-w-7xl mx-auto animate-fade-in-sm">
            <div className="flex items-center gap-4 mb-2 shrink-0">
                ...
                <div>
                    <h3 className="text-3xl font-light text-cyan-100 tracking-wide">parameter_weaving</h3>
                    <p className="text-base text-cyan-400/60">
                        Configure arguments for <strong className="font-mono text-cyan-200">{command.commandId}</strong>
                    </p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="mt-6 flex-1 flex flex-col min-h-0 relative">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {command.parameters.map((param) => (
                            ...
                            >
                                ...
                                <p className="text-sm text-cyan-400/50 mb-3 min-h-[40px]">{param.description}</p>
                                <div className="flex-1 flex flex-col justify-end">{renderInput(param)}</div>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="flex items-center justify-end gap-4 pt-6 mt-4 border-t border-cyan-500/10 bg-gray-900/95 backdrop-blur z-10">
                    ...
                    <Tooltip
                        label={isFormValid ? `Execute ${command.commandId}` : 'All required parameters must be filled'}
                    >
                        <div>
                            <button
                                type="submit"
