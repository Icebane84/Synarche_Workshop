...
def commit_transmutation(
    id: str, new_content: str, new_title: str, categorization: dict
):
    if not supabase:
    ...
    (
        supabase.table(TARGET_TABLE)
        .update({"content": new_content, "title": new_title, "metadata": new_metadata})
        .eq("id", id)
        .execute()
    )
...
if __name__ == "__main__":
    ...
    if cmd == "fetch":
        ...
        with (
            open(sys.argv[2], "r", encoding="utf-8") as f1,
            open(sys.argv[3], "r", encoding="utf-8") as f2,
        ):
        ...
        print(
            json.dumps(
                commit_transmutation(
                    sys.argv[2], sys.argv[3], content, json.loads(sys.argv[5])
                )
            )
        )
