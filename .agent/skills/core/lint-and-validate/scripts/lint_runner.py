...
def detect_project_type(project_path: Path) -> dict:
    ...
    if (project_path / "pyproject.toml").exists() or (
        ...
        if (
            shutil.which("pylint")
            or (project_path / ".venv_prs" / "Scripts" / "pylint.exe").exists()
        ):
            result["linters"].append(
                {
                    "name": "pylint",
                    "cmd": [sys.executable, "-m", "pylint", "axion-core"],
                }
            )

        # Check for flake8 (fallback)
        if (
            shutil.which("flake8")
            or (project_path / ".venv_prs" / "Scripts" / "flake8.exe").exists()
        ):
            result["linters"].append(
                {
                    "name": "flake8",
                    "cmd": [sys.executable, "-m", "flake8", "axion-core"],
                }
            )
    ...
    if (project_path / ".trunk").exists():
        result["linters"].append(
            {"name": "trunk", "cmd": ["trunk", "check", "--failing"]}
        )
